...
*Inspired by: [Lenny's Newsletter - writing product specs that engineers understand](https://www.lennysnewsletter.com/p/how-to-write-a-product-spec)*
...
*Inspired by: [Reforge - competitive intelligence as a core PM skill](https://www.reforge.com/blog/competitive-intelligence)*
...
A tool that takes any AI-generated analysis (a research summary, a market report, a user interview synthesis) and systematically verifies every quote, statistic, and citation against the original source material — flagging hallucinations before they end up in your deck.
...
1. Sarah reported that "updating three channels takes her entire day" — indicating severe workflow inefficiency.
2. Marcus mentioned spending "5-6 hours per week on stakeholder communications" — a significant time sink.
...
echo "Test data ready — note: quotes 2 and 4 in the analysis are fabricated"
```
...
- [ ] Quote 1 rated PARAPHRASED (close but "half a day" ≠ "entire day")
...
Take a real AI-generated summary you've used recently — a research synthesis, a competitive analysis, or a meeting summary — and run it through the verifier.
...
*Inspired by: [@lennysan on X — AI data analysis produces made-up quotes and invented evidence with total confidence](https://x.com/lennysan)*
...
A wrapper around the Claude API that monitors token usage in real-time during long working sessions, detects when context quality is degrading, and automatically summarizes older messages to keep the AI sharp — solving the "context rot" problem where AI gets worse the longer you talk to it.
...
- Implement "memory tiers" — recent (full), medium (summarized), old (keywords only)
...
*Inspired by: [@ttorres on X — AI gets worse the longer you talk to it, context rot degrades performance as conversations grow](https://x.com/ttorres)*
...
A tool that reads your product strategy document and tells you whether it contains actual strategic choices — trade-offs, constraints, specific bets — or if it's just dressed-up mission and vision statements. Because most strategy docs are aspirational fluff, not real strategy.
...
- Python 3.8+ installed
- ANTHROPIC_API_KEY set
- A product strategy document (or any strategy doc — even a blog post describing a company strategy)
...
## Strategic Context
We are the #3 player in project management tools. Competitors 1 and 2 own enterprise. We cannot win enterprise deals against them — they have 10x our sales team and SOC2 Type II.
...
- Enterprise features (SSO, SCIM, audit logs) — not this year
- Mobile app — our users work on laptops, mobile usage is <3%
- Integrations marketplace — we will support only Slack, GitHub, and Linear
...
*Inspired by: [@shreyas on X — most product strategy docs aren't real strategies because PMs start with mission/vision instead of strategic choices](https://x.com/shreyas)*
